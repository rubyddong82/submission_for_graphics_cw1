...
namespace
{
	...
	void blit_ex_solid_earth( benchmark::State& aState )
	{
		auto const width = std::uint32_t(aState.range(0));
		auto const height = std::uint32_t(aState.range(1));
		...
		for( auto _ : aState )
		{
			...
			blit_ex_solid( surface, *source, {500.f, 500.f} );
	...
	void blit_ex_memcpy_earth( benchmark::State& aState )
	{
		auto const width = std::uint32_t(aState.range(0));
		auto const height = std::uint32_t(aState.range(1));
		...
		for( auto _ : aState )
		{
			...
			blit_ex_memcpy( surface, *source, {500.f, 500.f} );
...
BENCHMARK( blit_ex_solid_earth )
	->Args( { 1920, 1080 } )
	->Args( { 7680, 4320 } )
;

BENCHMARK( blit_ex_memcpy_earth )
	->Args( { 1920, 1080 } )
	->Args( { 7680, 4320 } )
;
